...
public class RegistrationControllerTest {
	...
	public void testSubmitCustomerFailsValidation() throws Exception {
		this.mockMvc
				.perform(post("/registerUser").param("userId", user1.getUserId()).param("password", user1.getPassword())
						.param("name", user1.getName()).param("city", user1.getCity()).param("email", user1.getEmail())
						.param("phone", user1.getPhone()))
				.andExpect(model().attributeHasFieldErrors(command)).andExpect(view().name("register"))
				.andExpect(status().isOk()).andDo(print());
	}

	@Test
	public void testSubmitCustomerPassesValidation() throws Exception {
		this.mockMvc.perform(post("/registerUser").param("userId", user2.getUserId())
				.param("password", user2.getPassword()).param("name", user2.getName()).param("city", user2.getCity())
				.param("email", user2.getEmail()).param("phone", user2.getPhone()))
